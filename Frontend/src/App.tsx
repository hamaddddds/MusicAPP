    ...
    <div className="app-container" onContextMenu={(e) => {
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-section">
          ...
          <div className={`nav-item ${activeTab === "following" ? "active" : ""}`} onClick={() => setActiveTab("following")}><User size={20} /> Following {following.length > 0 && <span className="nav-count">{following.length}</span>}</div>
      ...
      <main className="main-content">
        ...
        {activeTab === "following" && (
          <div className="page">
            {!isAuth ? (
              <div className="empty-state big"><User size={44} /><p>Sign in to follow artists</p><span>Use the profile button to log in with GitHub.</span></div>
            ) : following.length ? (
              ...
              <div className="empty-state big"><User size={44} /><p>No artists followed yet</p><span>Tap Follow on any artist page to save them here.</span></div>
            )}
          </div>
        )}
        {activeTab === "artist" && (
                  ...
                  <div className="artist-page-meta">
                    ...
                    <div className="artist-page-actions">
                      ...
                      {artistView.artist?.artistId && (
                        <Button variant={isFollowing(artistView.artist.artistId) ? "default" : "ghost"} onClick={() => {
                          if (isFollowing(artistView.artist!.artistId!)) unfollow(artistView.artist!.artistId!);
                          else follow({ artistId: artistView.artist!.artistId!, name: artistView.artist!.name, thumb: pickArtwork(artistView.artist!.thumbnails) });
                        }}>
                          {isFollowing(artistView.artist.artistId) ? <><Check size={16} /> Following</> : <><Heart size={16} /> Follow</>}
                        </Button>
                      )}
                    </div>
                  </div>
